...
class GAN(tf.keras.Model):
    def __init__(self, args: argparse.Namespace) -> None:
        ...
        # - DONE takes vectors of shape `[args.z_dim]` on input
        # - DONE applies batch normalized dense layer with 1024 units and ReLU
        #   (do not forget about `use_bias=False` before every batch normalization)
        # - DONE applies batch normalized dense layer with `MNIST.H // 4 * MNIST.W // 4 * 64` units and ReLU
        # - DONE reshapes the current hidden output to `[MNIST.H // 4, MNIST.W // 4, 64]`
        # - DONE applies batch normalized transposed convolution with 32 filters, kernel size 4,
        ...
        # dense 1
        out = tf.keras.layers.Dense(1024, use_bias=False) (in_generator)
        out = tf.keras.layers.BatchNormalization()(out)
        out = tf.keras.layers.ReLU()(out)

        # dense 2
        out = tf.keras.layers.Dense(MNIST.H // 4 * MNIST.W // 4 * 64, use_bias=False) (out)
        out = tf.keras.layers.BatchNormalization()(out)
        out = tf.keras.layers.ReLU()(out)

        # reshape
        out = tf.keras.layers.Reshape([MNIST.H // 4, MNIST.W // 4, 64]) (out)

        # deconv
        out = tf.keras.layers.Conv2DTranspose(32, (4,4), strides=(2, 2), padding='same', activation=None, use_bias=False) (out)
        out = tf.keras.layers.BatchNormalization()(out)
        out = tf.keras.layers.ReLU()(out)

        # deconv 2 - output
        out_generator = tf.keras.layers.Conv2DTranspose(MNIST.C, (4,4), strides=(2, 2), padding='same', activation='sigmoid') (out)
        ...
        # - DONE computes batch normalized convolution with 32 filters, kernel size 5,
        #   same padding, and ReLU activation (again, do not forget about
        #   `use_bias=False` before every batch normalization).
        # - DONE max-pools with pool size 2 and stride 2
        # - DONE computes batch normalized convolution with 64 filters, kernel size 5,
        #   same padding, and ReLU activation
        # - DONE max-pools with pool size 2 and stride 2
        # - DONE flattens the current representation
        # - DONE applies batch normalized dense layer with 1024 units and ReLU activation
        ...
        out = tf.keras.layers.Conv2D(32, (5,5), strides=(1, 1), padding='same', activation=None, use_bias=False) (in_discriminator)
        out = tf.keras.layers.BatchNormalization()(out)
        out = tf.keras.layers.ReLU()(out)
        ...
        # conv2
        out = tf.keras.layers.Conv2D(64, (5,5), strides=(1, 1), padding='same', activation=None, use_bias=False) (out)
        out = tf.keras.layers.BatchNormalization()(out)
        out = tf.keras.layers.ReLU()(out)
        ...
        # dense
        out = tf.keras.layers.Dense(1024, use_bias=False) (out)
        out = tf.keras.layers.BatchNormalization()(out)
        out = tf.keras.layers.ReLU()(out)

        # output
        out_discriminator = tf.keras.layers.Dense(1, activation='sigmoid') (out)
