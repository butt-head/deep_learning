...
class VAE(tf.keras.Model):
    def __init__(self, args: argparse.Namespace) -> None:
        ...
        # - DONE flattens them
        # - DONE applies `len(args.encoder_layers)` dense layers with ReLU activation,
        ...
        in_encoder = tf.keras.layers.Input(shape=[MNIST.H, MNIST.W, MNIST.C])
        out_encoder = tf.keras.layers.Flatten() (in_encoder)

        for encoder_layer in args.encoder_layers:
            out_encoder = tf.keras.layers.Dense(encoder_layer, activation='relu') (out_encoder)

        # z_mean = tf.keras.layers.Dense(args.z_dim, activation='relu') (out_encoder)
        z_mean = tf.keras.layers.Dense(args.z_dim) (out_encoder)
        z_sd = tf.keras.layers.Dense(args.z_dim, activation='exponential') (out_encoder)

        out_encoder = (z_mean, z_sd)
        self.encoder = tf.keras.Model(inputs=in_encoder, outputs=out_encoder)

        # self.encoder.summary()

        # TODO: Define `self.decoder` as a `tf.keras.Model`, which
        # - DONE takes vectors of `[args.z_dim]` shape on input
        # - DONE applies `len(args.decoder_layers)` dense layers with ReLU activation,
        ...
        in_decoder = tf.keras.layers.Input(shape=[args.z_dim])

        out_decoder = in_decoder
        for decoder_layer in args.decoder_layers:
            out_decoder = tf.keras.layers.Dense(decoder_layer, activation='relu') (out_decoder)

        out_decoder = tf.keras.layers.Dense(MNIST.H * MNIST.W * MNIST.C, activation='sigmoid') (out_decoder)
        out_decoder = tf.keras.layers.Reshape([MNIST.H, MNIST.W, MNIST.C]) (out_decoder)
        
        self.decoder = tf.keras.Model(inputs=in_decoder, outputs=out_decoder)
    ...
    def train_step(self, images: tf.Tensor) -> Dict[str, tf.Tensor]:
        with tf.GradientTape() as tape:
            ...
            normal_dist = tfp.distributions.Normal(z_mean, z_sd) 
            z = normal_dist.sample(seed=self._seed)

            print("z:", z.shape)

            # TODO: Decode images using `z` (also passing `training=True` to the `self.decoder`).
            imgs_decoded = self.decoder(z, training=True)

            # TODO: Define `reconstruction_loss` using the `self.compiled_loss`.
            reconstruction_loss = self.compiled_loss(images, imgs_decoded)
            ...
            latent_loss = tf.math.reduce_mean( normal_dist.kl_divergence(self._z_prior) )

            # print("latent_loss:", latent_loss.shape)
