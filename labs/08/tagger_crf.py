...
class Model(tf.keras.Model):
    def __init__(self, args: argparse.Namespace, train: MorphoDataset.Dataset):#  -> None:
        ...
        words_embedded = tf.keras.layers.Embedding(vocab_size, args.we_dim)(words_indices)
        ...
        out = tf.keras.layers.Bidirectional(rnn_layer, merge_mode='sum')(words_embedded)
        ...
        predictions = tf.keras.layers.Dense(n_classes, activation=None)(out)
        ...
        self._crf_weights = self.add_weight(shape=(n_classes, n_classes), initializer='zeros', trainable=True)
    ...
    def crf_loss(self, gold_labels: tf.RaggedTensor, logits: tf.RaggedTensor):# -> tf.Tensor:
        ...
        logits_dense_t  = logits.to_tensor()
        gold_dense_t    = gold_labels.to_tensor()
        sequence_length = gold_labels.row_lengths()

        crf_loss = tfa.text.crf_log_likelihood(logits_dense_t, 
                                                gold_dense_t,
                                                sequence_length,
                                                self._crf_weights)
        return -tf.math.reduce_mean(crf_loss[0])
    ...
    def crf_decode(self, logits: tf.RaggedTensor):# -> tf.RaggedTensor:
        ...
        predictions = tfa.text.crf_decode(logits_dense_t, 
                                        self._crf_weights, 
                                        sequence_length)

        predictions = tf.RaggedTensor.from_tensor(predictions[0], lengths=sequence_length)
