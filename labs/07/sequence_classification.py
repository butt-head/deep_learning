...
class Model(tf.keras.Model):
    def __init__(self, args: argparse.Namespace): #-> None:
        ...
        if args.rnn_cell == 'SimpleRNN':
            rnn_layer = tf.keras.layers.SimpleRNN(args.rnn_cell_dim, return_sequences=True)

        out = rnn_layer(sequences)

        if args.hidden_layer != 0:
            out = tf.keras.layers.Dense(args.hidden_layer, activation='relu')(out)
            
        predictions = tf.keras.layers.Dense(1, activation=tf.nn.sigmoid)(out)
