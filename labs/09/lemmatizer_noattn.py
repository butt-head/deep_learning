...
class Model(tf.keras.Model):
    def __init__(self, args: argparse.Namespace, train: MorphoDataset.Dataset) -> None:
        ...
        # rnn_layer = tf.keras.layers.GRU(args.rnn_dim, return_sequences=True)
        rnn_layer = tf.keras.layers.GRU(args.rnn_dim)
        self.source_rnn = tf.keras.layers.Bidirectional(rnn_layer, merge_mode='sum')  ## !!! LAST ELEM
        # self.source_rnn = self.source_rnn[-1] !!!
        ...
        self.target_output_layer = tf.keras.layers.Dense(self.target_mapping.vocabulary_size(), activation=None)  ## otu - logits
    ...
    class DecoderTraining(tfa.seq2seq.BaseDecoder):
        ...
        @property
        def batch_size(self):
            # TODO: Return the batch size of `self.source_states` as a *scalar* number;    !!! self.lemmatizer.source_states
        ...
        def initialize(self, layer_inputs, initial_state=None):
            ...
            finished = tf.fill([self.batch_size], False)   # [self.batch_size] !!!
            ...
            inputs = self.lemmatizer.target_embedding(tf.fill([self.batch_size], MorphoDataset.Factor.BOW))  # [self.batch_size] !!!
    ...
    class DecoderPrediction(DecoderTraining):
        ...
        def output_size(self):
            ...
            # return tf.TensorShape(tf.constant(1))
        ...
        def step(self, time, inputs, states, training):
            ...
            outputs = tf.argmax(outputs, axis=-1, output_type=tf.int32)   ### !!! which axis??? nevracíme floaty, ale inty
    ...
    def call(self, inputs, targets=None):
        ...
        if targets is not None:
            ...
            maximum_iterations=tf.cast(source_charseqs.bounding_shape(1) + 10, tf.int32)
            DecoderPrediction = self.DecoderPrediction(self, maximum_iterations=maximum_iterations)
